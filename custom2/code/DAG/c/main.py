...
async def call_service_d(value: int):
    async with semaphore:
        ...
        # b -> c로 POST 요청
        response = await client.post("http://serviced:11003/d", json=data_to_send)
...
async def c_endpoint(request: Request):
    ...
    # c 서비스 호출
    try:
        result_from_c = await call_service_d(value)
        # c 결과를 그대로 a에게 반환
